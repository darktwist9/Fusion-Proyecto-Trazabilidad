@php
    ...
    $rutaPrefijo = $rutaPrefijo ?? 'almacen-mayorista.traslados-planta';
@endphp
...
<div class="may-rec-panel">
    ...
    @if($estado['puede_firmar'] ?? false)
        <a href="{{ $estado['url_cierre'] ?? route($rutaPrefijo.'.cierre.panel', $ruta) }}"
           class="btn btn-warning btn-block font-weight-bold">
            <i class="fas fa-file-signature mr-1"></i> Firmar recepción
        </a>
    ...
    @elseif(($estado['clave'] ?? '') === 'en_camino' && $simulacionActiva && \App\Support\RutaTiempoRealAcceso::puedeVerTrasladoPlantaMayorista(auth()->user(), (int) $ruta->rutadistribucionid))
        <a href="{{ route('logistica.rutas-tiempo-real.show', ['tipo' => 'planta_mayorista', 'id' => $ruta->rutadistribucionid]) }}"
